...
const PIA_GROUP_MEMBERS = {
  "AUTO-FAST": PIA_ENDPOINTS.filter((endpoint) => endpoint.hot).map((endpoint) => endpoint.name),
  "AUTO-SAFE": PIA_ENDPOINTS.filter((endpoint) => endpoint.hot).map((endpoint) => endpoint.name),
  "LB-HLS": PIA_ENDPOINTS.filter((endpoint) => endpoint.hot).map((endpoint) => endpoint.name),
  "LB-STICKY": PIA_ENDPOINTS.filter((endpoint) => endpoint.hot).map((endpoint) => endpoint.name),
  "LB-ROBIN": PIA_ENDPOINTS.filter((endpoint) => endpoint.hot).map((endpoint) => endpoint.name),
  "LB-CONSISTENT": PIA_ENDPOINTS.filter((endpoint) => endpoint.hot).map((endpoint) => endpoint.name),
  "OV-ALL": PIA_ENDPOINTS.map((endpoint) => endpoint.name),
  "OV-ASIA": PIA_ENDPOINTS.filter((endpoint) => ["tw","ph","sg","mo","hk","cn","jp","kr","my","id","vn","in","kh","mn","np","bd","lk"].includes(endpoint.id)).map((endpoint) => endpoint.name),
  "OV-MIDDLE-EAST": PIA_ENDPOINTS.filter((endpoint) => ["ae","qa","sa","il","tr","eg"].includes(endpoint.id)).map((endpoint) => endpoint.name),
  "OV-EUROPE": PIA_ENDPOINTS.filter((endpoint) => ["nl-netherlands","de-berlin","de-frankfurt","uk-london","uk-manchester","uk-southampton","fr","es-madrid","es-valencia","it-milano","am","ge","kz","ad","al","at","ba","be","bg","ch","cy","cz","dk-copenhagen","ee","fi-helsinki","gr","hr","hu","ie","im","is","li","lt","lu","lv","mc","md","me","mk","mt","no","pl","pt","ro","rs","se-stockholm","si","sk","ua"].includes(endpoint.id)).map((endpoint) => endpoint.name),
  "OV-NORTH-AMERICA": PIA_ENDPOINTS.filter((endpoint) => endpoint.id.startsWith("us-") || endpoint.id.startsWith("ca-") || endpoint.id === "gl").map((endpoint) => endpoint.name),
  "OV-LATIN-AMERICA": PIA_ENDPOINTS.filter((endpoint) => ["mx","br","ar","cl","co","bo","bs","cr","ec","gt","pa","pe","uy","ve"].includes(endpoint.id)).map((endpoint) => endpoint.name),
  "OV-OCEANIA": PIA_ENDPOINTS.filter((endpoint) => endpoint.id.startsWith("au-") || endpoint.id === "nz").map((endpoint) => endpoint.name),
  "OV-AFRICA": PIA_ENDPOINTS.filter((endpoint) => ["za","ng","ma","dz"].includes(endpoint.id)).map((endpoint) => endpoint.name)
};
